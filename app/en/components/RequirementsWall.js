...
// S3 要件の壁 + S4 転換
//
// この2つは同一ファイル・同一背景(#0A0F1E)に置く。背景を切り替えると
...
function SourceLink({ href, label }) {
  return (
    <a
      ...
      className="inline-flex items-center gap-1.5 text-[#7db3ff] hover:text-white text-xs font-semibold transition-colors underline decoration-[#7db3ff]/40 underline-offset-4 hover:decoration-white"
    >
...
function WallCard({ item }) {
  return (
    <div className="h-full flex flex-col border border-white/15 rounded-2xl p-6 bg-white/[0.03]">
      <div className="text-[#7db3ff] font-black text-xl leading-none mb-5">
        {item.number}
      </div>
      ...
      {item.quote ? (
        <blockquote className="mb-4">
          ...
          <cite className="not-italic block text-white/55 text-[11px] leading-relaxed mt-2.5">
            {item.original}
          </cite>
        </blockquote>
      ) : (
        <p className="text-white text-[15px] md:text-base font-bold leading-relaxed mb-4">
          {item.headline}
        </p>
      )}
      ...
      <p className="text-white/70 text-sm leading-relaxed">{item.meaning}</p>
...
function Group({ label, title, subline, items, showChain }) {
  return (
    <div className="relative pl-5 md:pl-7 border-l-2 border-[#0066FF]/40">
      <p className="text-[#7db3ff] text-[11px] font-bold tracking-[0.3em] uppercase mb-2">
        {label}
      </p>
      <h3 className="text-white font-extrabold text-xl md:text-2xl leading-snug mb-1.5">
        {title}
      </h3>
      <p className="text-white/55 text-sm mb-8">{subline}</p>
        ...
        {items.map((item, i) => (
          ...
          >
            ...
            {showChain && i < items.length - 1 && (
              <div
                className="flex md:hidden justify-center py-2 text-[#7db3ff] text-xl"
                aria-hidden="true"
              >
                ↓
              </div>
            ...
            {showChain && i < items.length - 1 && (
              <div
                className="hidden md:flex absolute top-1/2 -right-[13px] -translate-y-1/2 z-10 text-[#7db3ff] text-lg"
                aria-hidden="true"
              >
                →
              </div>
  ...
  return (
    <>
      ...
      <section
        id="requirements"
        data-bg="#0A0F1E"
        className="bg-[#0A0F1E] pt-24 md:pt-36 pb-20 md:pb-28 px-6 md:px-16 scroll-mt-20"
      >
        <div className="max-w-6xl mx-auto">
          ...
          <motion.p
            className="text-white/70 text-base md:text-lg leading-relaxed max-w-2xl mb-16"
            initial={{ opacity: 0 }}
          ...
          >
            ...
            <p className="text-white/70 text-sm md:text-base text-center max-w-xl leading-relaxed">
          ...
          <motion.div
            className="mt-16 md:mt-20 border border-white/20 rounded-2xl p-8 md:p-10 bg-white/[0.05]"
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.6 }}
          >
            <p className="text-[#7db3ff] text-[11px] font-bold tracking-[0.3em] uppercase mb-4">
              The floor under all six
            </p>
            <p className="text-white font-bold text-lg md:text-xl mb-7">
              And all six of them stand on the same floor.
            </p>

            <ul className="space-y-5 mb-7">
              <li className="flex gap-4">
                <span className="text-[#7db3ff] flex-shrink-0" aria-hidden="true">—</span>
          ...
          <motion.div
            ...
            transition={{ duration: 0.6 }}
          >
            <p className="text-white/60 text-[10px] font-bold tracking-[0.25em] uppercase mb-3">
      ...
      <section
        data-bg="#0A0F1E"
        className="bg-[#0A0F1E] py-20 md:py-28 px-6 md:px-16 overflow-hidden"
      >
        <div className="max-w-4xl mx-auto text-center">
          {/* 6→2→1 の収束。S3で膨らんだ認知負荷が目に見えて縮む */}
          <motion.div
            className="flex items-center justify-center gap-4 md:gap-8 mb-12 text-[10px] md:text-xs font-bold tracking-[0.2em] uppercase"
            initial={{ opacity: 0 }}
            whileInView={{ opacity: 1 }}
            viewport={{ once: true }}
          ...
          >
            <div className="flex flex-col items-center gap-2.5">
              ...
              <span className="text-white/65">6 requirements</span>
            </div>
            <span className="text-white/45 text-base" aria-hidden="true">→</span>
            <div className="flex flex-col items-center gap-2.5">
              ...
              <span className="text-white/65">2 walls</span>
            </div>
            <span className="text-white/45 text-base" aria-hidden="true">→</span>
            <div className="flex flex-col items-center gap-2.5">
              <span className="block w-5 h-7 bg-[#0066FF]" aria-hidden="true" />
              <span className="text-[#7db3ff]">1 question</span>
            </div>
          </motion.div>
